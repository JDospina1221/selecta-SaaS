...
export class OrderService {
  ...
  // Magia pura: El subtotal se calcula solo cada vez que el carrito cambia
  subtotal = computed(() => {
    return this.cart().reduce((sum, item) => sum + (item.product.price * item.quantity), 0);
  });

  // Por ahora el total es igual al subtotal (después le podemos meter impuestos o propinas)
