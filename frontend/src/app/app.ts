...
export class App implements OnInit {
  ...
  // --- CONSTRUCTOR Y NUEVOS PRODUCTOS ---
  isEditProductModalOpen = signal(false); editingProduct = signal<any>(null); editPrice = signal(0); editRecipe = signal<any[]>([]); 
  isNewProdModalOpen = signal(false); newProdName = signal(''); newProdPrice = signal(0); newProdCategory = signal('Hamburguesas'); // <-- NUEVAS VARIABLES
  ...
  saveProductChanges() {
    ...
    this.adminService.updateProduct(product.id, { price: this.editPrice(), recipe: this.editRecipe() }).subscribe({
      next: () => { this.adminService.loadProducts(this.currentUser()?.tenantId || 'sociedad_selecta_001'); this.closeEditProduct(); }
    });
  }

  // --- CREAR Y ELIMINAR DEL MENÚ ---
  ...
  saveNewProduct() {
    const user = this.currentUser(); if (!user || !this.newProdName() || this.newProdPrice() <= 0) return alert('Llene todos los datos.');
    const payload = { tenantId: user.tenantId || 'sociedad_selecta_001', name: this.newProdName(), category: this.newProdCategory(), price: this.newProdPrice() };
    this.adminService.addProduct(payload).subscribe({ next: () => { this.adminService.loadProducts(payload.tenantId); this.closeNewProdModal(); } });
  }
  deleteProduct(id: string) {
    if (confirm('¿Estás seguro que deseas eliminar este producto del menú? No se podrá recuperar.')) {
      this.adminService.deleteProduct(id).subscribe({ next: () => { this.adminService.loadProducts(this.currentUser()?.tenantId || 'sociedad_selecta_001'); } });
    }
  }
  ...
  saveInventory() {
    const user = this.currentUser(); if (!user || !this.invName()) return;
    const payload = { tenantId: user.tenantId || 'sociedad_selecta_001', name: this.invName(), category: this.invCategory(), stock: this.invStock(), cost: this.invCost(), unit: this.invUnit() };
    if (this.editingInv()) this.inventoryService.updateInventoryItem(this.editingInv().id, payload).subscribe(() => { this.inventoryService.loadInventory(payload.tenantId); this.closeInvModal(); });
    else this.inventoryService.addInventoryItem(payload).subscribe(() => { this.inventoryService.loadInventory(payload.tenantId); this.closeInvModal(); });
  }
  ...
  setAdminView(view: string) {
    this.adminCurrentView.set(view); const user = this.currentUser(); if (!user) return;
